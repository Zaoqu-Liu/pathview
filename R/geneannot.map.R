  ...
  if (class(res)[1] == "data.frame") {
    ...
    if (unique.map) {
      if (length(out.type) == 1) {
        umaps <- tapply(res[, out.type], res[, in.type], paste, sep = "", collapse = "; ")
      } else {
        umaps <- apply(res[, out.type], 2, function(x) tapply(x, res[, in.type], function(y) paste(unique(y), sep = "", collapse = "; ")))
      }
      umaps <- cbind(umaps)
