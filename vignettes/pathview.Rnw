...
data(gse16873.d)
pv.out <- pathview(
  gene.data = gse16873.d[, 1], pathway.id = "04110",
...
gse16873.t <- apply(gse16873.d, 1, function(x) {
  t.test(x,
...
pv.out <- pathview(
  gene.data = sel.genes, cpd.data = sel.cpds,
...
pv.out <- pathview(
  gene.data = sel.genes, cpd.data = sim.cpd.data,
...
cpd.cas <- sim.mol.data(
  mol.type = "cpd", id.type = cpd.simtypes[2],
  nmol = 10000
)
gene.ensprot <- sim.mol.data(
  mol.type = "gene", id.type = gene.idtype.list[4],
  nmol = 50000
)
pv.out <- pathview(
  gene.data = gene.ensprot, cpd.data = cpd.cas,
...
head(eco.dat.kegg)
pv.out <- pathview(
  gene.data = eco.dat.kegg, gene.idtype = "kegg",
...
head(eco.dat.entrez)
pv.out <- pathview(
  gene.data = eco.dat.entrez, gene.idtype = "entrez",
...
head(eco.dat.kegg)
pv.out <- pathview(
  gene.data = eco.dat.symbol, gene.idtype = "symbol",
...
pv.out <- pathview(
  gene.data = ko.data, pathway.id = "04112",
...
pv.out.list <- pathview(
  gene.data = gse16873.d[, 1:3], pathway.id = path.ids2,
  species = "hsa"
)
@ 
